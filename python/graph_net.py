...
class GNNBlock(nn.Module):
    def __init__(self,
        ...
        # create MLPs
        self.affine_edge = ExplicitMLP(in_edge_feats, hidden_feats, hidden_feats, 2, activation, False)
        self.affine_node_src = nn.Linear(in_node_feats, hidden_feats)
        self.affine_node_dst = nn.Linear(in_node_feats, hidden_feats)

        self.phi = ExplicitMLP(hidden_feats, in_edge_feats, hidden_feats, 2, activation, True)
        ...
        self.theta = ExplicitMLP(hidden_feats, out_node_feats, hidden_feats, 1, activation, True)

    def forward(self,
                g: dgl.DGLGraph,
                node_feats: torch.Tensor) -> torch.Tensor:
        # copy node features
        h = node_feats.clone()
...
class GNNModel(nn.Module):
    def __init__(self,
        ...
        for i in range(num_blocks):
            if i == 0:
                self.gn_blocks.append(GNNBlock(in_node_feats,
                                               in_edge_feats,
                                               out_node_feats,
                                               hidden_feats,
                                               activation,
                ...
                self.gn_blocks.append(GNNBlock(out_node_feats,
                                               in_edge_feats,
                                               out_node_feats,
                                               hidden_feats,
                                               activation,
...
class MDNet(nn.Module):
    def __init__(self,
        ...
        self.length_avg = nn.Parameter(torch.tensor([0.]), requires_grad=False)
        ...
        self.node_feats = nn.Parameter(torch.randn(1, self.embed_feats), requires_grad=True)

        # create edge encoder
        in_edge_feats = 3 + 1 + len(self.edge_expand.centers)
        self.edge_encoder = ExplicitMLP(in_edge_feats, self.embed_feats, hidden_feats, 3, 'gelu', False)

        # create node decoder
        self.node_decoder = ExplicitMLP(embed_feats, 3, hidden_feats, 2, 'gelu', False)

        # create GNN
        self.net = GNNModel(in_node_feats=in_node_feats,
    ...
    def forward(self,
        ...
        edge_feats = torch.concat((graph.edata['unit_vec'], 
                                   distance,
                                   self.edge_expand(distance)), -1)
