...
class MDDataset(DGLDataset):
    def __init__(
        ...
        """
        self.r_cut = r_cut
        self.trajectories = range(
            traj_index["start"], traj_index["end"], traj_index["step"]
        )
        self.snapshots = range(
            snap_index["start"], snap_index["end"], snap_index["step"]
        )
    ...
    def process(self):
        ...
        for traj in tqdm(self.trajectories):
            for snap in self.snapshots:
                filename = os.path.join(self.raw_dir, f"traj_{traj}", f"dump.lj.{snap}")
    ...
    def save(self):
        dgl.save_graphs(self.data_name, self.graphs, {"labels": self.labels})
    ...
    def _generate_graph(self, atoms: Atoms = None) -> dgl.DGLGraph:
