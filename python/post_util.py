...
def read_CNA(filename, steps):
    ...
    npar = np.array(
        re.findall("[+-]? *(?:\d+(?:\.\d*)?|\.\d+)(?:[eE][+-]?\d+)?", lines[3].strip()),
        dtype=int,
    )
    nat = npar[0]
...
def local_sq(
    pos, h=None, N=200, wdt=500, cs=3, ms=30, dump=False, correction_grid=None
):
    _, array_sq = s_q_from_pos_smear_array(
        pos, h=h, N=N, wdt=wdt, cs=cs, ms=ms, dump=dump, correction_grid=correction_grid
    )
    return array_sq
...
def get_kpoints(
    ...
    system,
    delta_x=0.0005,
    points=[r"$\Gamma$", r"$X$", r"$W$", r"$K$", r"$\Gamma$", r"$L$"],
):
    ...
    for i in range(nKin - 1):
        ...
        segment_points = np.linspace(
            kpointsIn[i], kpointsIn[i + 1], nInterp_per_segment[i], endpoint=False
        )
...
def get_fractional_hess(hess, natoms_cell, nnc, n_unit_cells):
    fractional_hess = np.zeros(
        (natoms_cell * 3, natoms_cell * 3, nnc * n_unit_cells**3)
    )
    for i in range(nnc * n_unit_cells**3):
        fractional_hess[:, :, i] = hess[
            i * natoms_cell * 3 : (i + 1) * natoms_cell * 3, : natoms_cell * 3
        ]
