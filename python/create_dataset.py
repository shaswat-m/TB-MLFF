...
parser.add_argument(
    "--save_dir", required=False, type=str, default=os.getcwd(), help="save directory"
)
parser.add_argument(
    "--start_idx_traj",
    required=False,
...
parser.add_argument(
    "--num_atom", required=False, type=int, help="number of atoms", default=256
)
parser.add_argument(
    "--density", required=False, type=float, help="LJ density of Argon", default=0.844
)
parser.add_argument(
    "--temperature_init",
    required=False,
...
parser.add_argument(
    "--pressure", required=False, type=float, help="pressure (bar)", default=137.3
)
parser.add_argument(
    "--num_vacancy", required=False, type=int, help="Create vacancies", default=0
)
parser.add_argument(
    "--potential_file",
    required=False,
...
parser.add_argument(
    "--material", required=False, type=str, help="material", default="Ar"
)
parser.add_argument(
    "--time_step", required=False, type=float, help="time step (ps)", default=0.01078
)
parser.add_argument(
    "--run_step", required=False, type=int, help="run step ", default=200000
)
parser.add_argument(
    "--dump_freq", required=False, type=int, help="dump frequency ", default=100
)
...
class RunMD:
    ...
    def run(self):
        # create instances
        cmdargs = ["-log", f"{self.filename_log}.{self.idx_traj}", "-screen", "none"]
        ...
        self.lmp.command(
            f"region box block 0 {self.num_replicate} \
        ...
        self.lmp.command(
            f"velocity all create {self.temp_obj} {self.random} \
        ...
        if args.potential_file == "lj":
            ...
            self.lmp.command(
                f"pair_coeff 1 1 {self.epsilon} {self.sigma} {self.cutoff}"
            )
        ...
        self.lmp.command(
            f"fix 1 all npt temp {self.temp_obj} {self.temp_obj} {self.npt_thermo_freq} \
        ...
        # run NVT ensemble (1)
        self.lmp.command(
            f"fix 1 all nvt temp {self.temp_obj} {self.temp_obj} {self.nvt_thermo_freq} \
                           tchain {self.num_chains} tloop {self.num_mtk}"
        )
        self.lmp.command(f"run {self.num_step}")
        ...
        self.lmp.command(
            f"dump 1 all custom {self.dump_freq} {self.filename_dump} \
                           id type x y z vx vy vz fx fy fz"
        )
        self.lmp.command(
            f"dump 2 all custom {self.dump_freq} {self.filename_dump_one} \
        ...
        self.lmp.command(f"unfix 1")
        self.lmp.command(
            f"fix 1 all nvt temp {self.temp_init} {self.temp_final} {self.nvt_thermo_freq} \
                           tchain {self.num_chains} tloop {self.num_mtk}"
        )
