...
from nvt_integrator import (
    Hack1stHalfNoseHooverIntegrator,
    Hack2ndHalfNoseHooverIntegrator,
)
from openmm import app, unit
from tqdm import tqdm
from train import LightDataModule, LightModel
...
class Test:
    ...
    def generate_graph(self, atoms: Atoms = None) -> dgl.DGLGraph:
        ...
        src_ids, dst_ids, distance, unit_vec = neighborlist.neighbor_list(
            "ijdD", atoms, self.r_cut
        )
        ...
        graph.edata["unit_vec"] = torch.from_numpy(
            np.divide(unit_vec, distance)
        ).float()

        return graph

    def init_gnn_model(
        self, filename: str = None, train_params: dict = None, model_params: dict = None
    ) -> pl.LightningModule:
        model = LightModel(train_params=train_params, model_params=model_params)
        model = model.load_from_checkpoint(
            filename, train_params=train_params, model_params=model_params
        )
        model.eval()
...
class TestMD(Test):
    ...
    def setup(self, args):
        ...
        os.system(
            ...
            % (
                os.path.join(
                    self.test_params["save_dir"], self.test_params["dump_filename"]
                ),
        ...
        os.system(
            "cp %s %s ;"
            % (
                os.path.join(self.test_params["save_dir"], "best.ckpt"),
                os.path.join(self.save_dir, "best.ckpt"),
            )
        )
        self.dump_filename = os.path.join(
            self.train_params["save_dir"], self.test_params["dump_filename"]
        )
        self.log_filename = os.path.join(
            self.save_dir, self.test_params["log_filename"]
        )
    ...
    def run_lj(self):
        ...
        for _ in range(num_atoms):
            ...
            nb.addParticle(
                0, SIGMA * unit.angstrom, EPSILON * unit.kilocalorie_per_mole
            )
        ...
        integrator = mm.NoseHooverIntegrator(
            temperature, thermo_freq, timestep, chain_length, 5
        )
        ...
        simulator = app.Simulation(
            topology=topology, system=system, integrator=integrator, platform=platform
        )
        ...
        for t in range(1, total_timestep + 1):
            ...
            if t % dump_freq == 0:
                ...
                io.write(
                    filename=os.path.join(self.save_dir, f"data.lj.{t}"),
                    images=self.atoms,
                    format="lammps-data",
                    velocities=True,
                )
    ...
    def run_gnn(self):
        ...
        model = self.init_gnn_model(filename_ckpt, self.train_params, self.model_params)
        ...
        integrator1 = Hack1stHalfNoseHooverIntegrator(
            system=system,
        ...
        integrator2 = Hack2ndHalfNoseHooverIntegrator(
            system=system,
            temperature=temperature,
            collision_frequency=thermo_freq,
            chain_length=chain_length,
        ...
        if self.test_params["device"] == "gpu":
            device = "CUDA"
        platform = mm.Platform.getPlatformByName(device)
        dummy_simulator = app.Simulation(
            topology=topology,
            system=system,
        ...
        force_unit = (
            1
            / (aseUnits.kJ / aseUnits.mol / aseUnits.nm)
        ...
        with torch.no_grad():
            ...
            self.atoms.set_velocities(vel / (aseUnits.fs * 1000))
            io.write(
                filename=os.path.join(self.save_dir, f"data.lj.0"),
                images=self.atoms,
                format="lammps-data",
                velocities=True,
            )
            ...
            for t in range(1, total_timestep + 1):
                ...
                pos = dummy_state.getPositions(asNumpy=True).value_in_unit(
                    unit.angstrom
                )
                ...
                vel = dummy_state.getVelocities(asNumpy=True).value_in_unit(
                    unit.angstrom / unit.picosecond
                )
                self.atoms.set_velocities(vel / (aseUnits.fs * 1000))
                if t < total_timestep:
    ...
    def run(self):
        if self.ff_model == "lj":
            ...
            raise ValueError(
                f"lj or gnn only acceptable for force field: \
                               {self.ff_model}"
            )
...
class TestMS(Test):
    ...
    def setup(self, args: dict = None) -> None:
        ...
        self.dump_filename = os.path.join(
            self.save_dir, self.test_params["dump_filename"]
        )
    ...
    def test_hessian(self, dx: float = 1e-6) -> None:
        ...
        model = self.init_gnn_model(
            self.filename_ckpt, self.train_params, self.model_params
        )
        avg_force = model.avg_force.numpy()
        std_force = model.std_force.numpy()
        ...
        with torch.no_grad():
            for i in tqdm(range(k_dof)):
                ...
                self.k_mat_gnn[i, :] = k_mat_gnn_row.reshape(
                    -1,
                )

        # save hessian matrix
        np.savez(
            self.hess_filename, hess_gnn=self.k_mat_gnn, hess_lammps=self.k_mat_lmp
        )
    ...
    def test_invariant(self) -> None:
        ...
        pos_trans = pos.copy() + np.repeat(
            1e1 * np.random.rand(3).reshape(1, 3), num_atoms, axis=0
        )
    ...
    def test_parity(self):
        dm = LightDataModule(
            train_params=self.train_params, data_params=self.data_params
        )
        dm.setup()
        ...
        with torch.no_grad():
            ...
            for batch in val_dataloader:
                ...
                count += 1
        err = abs(
            np.concatenate(forces_pred).reshape(-1)
            - np.concatenate(forces_refs).reshape(-1)
        )
        err2 = np.concatenate(forces_pred).reshape(-1) - np.concatenate(
            forces_refs
        ).reshape(-1)
        err3 = (
            np.concatenate(forces_refs).reshape(-1)
            - (np.concatenate(forces_refs).reshape(-1)).mean()
        )
        sse = (err2**2).sum()
        sst = (err3**2).sum()
        r2 = 1 - sse / sst
    ...
    def run(self):
        if self.test_name == "hessian":
            ...
            raise ValueError(
                f"hessian, parity or invariant only acceptable: \
                               {self.test_name}"
            )
