...
def load_atom_data(
    ...
    nparticles = np.genfromtxt(
        filename, skip_header=skip_to_np, dtype=np.int, max_rows=1
    ).item(0)
    box = np.genfromtxt(filename, skip_header=skip_to_box, dtype=np.double, max_rows=3)[
        :, :2
    ]
    if h_full:
    ...
    rawdata = np.genfromtxt(
        filename, skip_header=skip_to_pos, dtype=np.double, max_rows=nparticles
    )
...
def load_dumpfile_atom_data(
    ...
    npar = np.array(
        re.findall(
            "[+-]? *(?:\d+(?:\.\d*)?|\.\d+)(?:[eE][+-]?\d+)?", lines[skip_to_np].strip()
        ),
        dtype=int,
    )
    nparticles = npar[0]
    ...
    for i in range(ndum):
        ...
        if h_full:
            pos[i * nparticles : (i + 1) * nparticles] = np.dot(
                pos[i * nparticles : (i + 1) * nparticles], np.diag(h[i, :])
            )
        else:
            hin = h[:, 1, i] - h[:, 0, i]
            pos[i * nparticles : (i + 1) * nparticles] = np.dot(
                pos[i * nparticles : (i + 1) * nparticles], np.diag(hin)
            )
    if verbose:
        print("Nparticles = %d" % (nparticles))
        print("Simulation Cell")
...
def load_dumpfile_atom_data_fast(
    filename,
    total_steps,
    dump_frequency,
    verbose=True,
...
def load_dumpfile_velocity(
    filename, total_steps, dump_frequency, verbose=True, at_type=1
):
    """Load LAMMPS .data type configuration files
...
def load_dumpfile_atom_data_binary(
    filename, total_steps, dump_frequency, verbose=True, at_type=1, family=None, fill=0
):
    """Load LAMMPS .data type configuration files
    ...
    for i in range(ndum):
        ...
        skip_to_box = (i + ct) * nparticles + (i + ct + 1) * 9 - 4
        box = np.genfromtxt(
            filename, skip_header=skip_to_box, dtype=np.double, max_rows=3
        )[:, :2]
...
def load_dumpfile_atom_data_binary_fast(
    filename,
    total_steps,
    dump_frequency,
    h_full=True,
...
def bond_vector_binary_par(pos, natom_a, rv, vectorization=False):
    ...
    pos_b = pos[natom_a:-1, :]
    h = np.diag(pos[-1, :])
    nn_a, nindex_a, nn_b, nindex_b = verletlist_binary(
        pos_a, pos_b, h=h, rv=rv, vectorization=vectorization
    )
...
def compute_angle(v1, v2):
    return (
        180.0
        - (np.arccos(np.sum(v1 * v2) / (v1**2).sum() ** 0.5 / (v2**2).sum() ** 0.5))
        * 180
        / np.pi
    )
...
def compute_angle_distribution(filename, rv=1.1):
    ...
    skip_to_box = 7
    skip_to_pos = 29
    pos_a, pos_b, h, atoms_a, atoms_b = load_atom_data_binary(
        filename,
        skip_to_np,
        skip_to_box,
        skip_to_pos,
...
def s_q_from_g_r(q_array, r_array, g_array, r0, rho):
    ...
    for i in range(q_array.shape[0]):
        ...
        int_0_r0 = (r0 * q * np.cos(r0 * q) - np.sin(r0 * q)) / np.power(
            q, 3
        )  # integral from 0 to r0
        s_array[i] = 1.0 + 4.0 * np.pi * rho * (
            int_0_r0 + np.trapz(np.multiply(g_array - 1.0, r_sinqr), x=r_array) / q
        )
    return s_array
...
def s_q_from_pos(q_array, pos, h, rc, rho, nnlist=None):
    ...
    for i in range(q_array.shape[0]):
        q = q_array[i]
        sinqr_div_r = np.divide(
            np.sin(q * r_array), r_array
        )  # np.exp(-1j * r_array* q)
        int_0_rc = (rc * q * np.cos(rc * q) - np.sin(rc * q)) / np.power(
            q, 3
        )  # integral from 0 to rc
        s_array[i] = (
            1.0 + 4.0 * np.pi * rho * int_0_rc + np.sum(sinqr_div_r) / (nparticles * q)
        )  # np.sum(sinqr_div_r)/(nparticles)
    return s_array.real
...
def s_q3_from_pos(
    q3_flat, pos, nparticles, intensity=False, smear=False, ddq=0, r_sq=None
):
...
def s_q3_from_pos_par(
    ...
    for i in range(q3_flat.shape[0]):  # note: range -> prange
        ...
        if q < 1e-10:
            continue
        if smear:
            exp_miqr = np.exp(-1j * np.dot(r_array, q3.T)).flatten() * np.exp(
                -(0.5 * ddq**2) * r_sq**2
            )
            ...
            s_flat[i] = (
                1.0 + 4.0 * np.pi * rho * int_0_rc + np.sum(exp_miqr) / nparticles
            )
    return s_flat.real
...
def I_q3_from_pos_par(
    q3_flat, r_array, rc, rho, nparticles, ff, smear=False, ddq=0, r_sq=None
):
    """Get s(q) from positions over the entire 2D grid
    ...
    for i in range(q3_flat.shape[0]):  # note: range -> prange
        ...
        if smear:
            exp_miqr = (
                (((2 * np.pi) ** 0.5 * ddq) ** 3)
                * np.exp(-1j * np.dot(r_array, q3.T)).flatten()
                * np.exp(-(0.5 * ddq**2) * r_sq**2)
            )
            int_0_rc = 0 * (
                -rc
                * ((2 * np.pi) ** 0.5)
                * np.cos(rc * q)
                * np.exp(-0.5 * (ddq**2) * rc**2)
                / ddq
            )  # integral from 0 to rc
            s_flat[i] = (
                (ddq * (2 * np.pi) ** 0.5) ** 3
                + 4.0 * np.pi * rho * int_0_rc
                + np.sum(exp_miqr) / nparticles
            ) * ff[i] ** 2
        else:
            exp_miqr = np.exp(-1j * np.dot(r_array, q3.T))
            int_0_rc = (rc * q * np.cos(rc * q) - np.sin(rc * q)) / np.power(
...
def ISF_from_pos_par(
    posit,
    s_ref=np.zeros((10, 10, 10)),
    N=200,
    wdt=500,
    cs=3,
    ms=30,
    ind_need=np.array([0, 2]),
    dump=False,
    grid=False,
):
    ...
    for i in range(NP):
        ...
        if (kx >= cs) and (kx < N - cs):
            ...
            indx = np.append(
                np.linspace(N - (cs - kx), N - 1, cs - kx, dtype=int),
                np.linspace(0, kx + cs, 2 * cs + 1 - (cs - kx), dtype=int),
            )
        elif kx >= N - cs:
            indx = np.append(
                np.linspace(kx - cs, N - 1, cs + N - kx, dtype=int),
        ...
        if (ky >= cs) and (ky < N - cs):
            ...
            indy = np.append(
                np.linspace(N - (cs - ky), N - 1, cs - ky, dtype=int),
                np.linspace(0, ky + cs, 2 * cs + 1 - (cs - ky), dtype=int),
            )
        elif ky >= N - cs:
            indy = np.append(
                np.linspace(ky - cs, N - 1, cs + N - ky, dtype=int),
        ...
        if (kz >= cs) and (kz < N - cs):
            ...
            indz = np.append(
                np.linspace(N - (cs - kz), N - 1, cs - kz, dtype=int),
                np.linspace(0, kz + cs, 2 * cs + 1 - (cs - kz), dtype=int),
            )
        elif kz >= N - cs:
            indz = np.append(
                np.linspace(kz - cs, N - 1, cs + N - kz, dtype=int),
...
def f_weight_array(g, r, rc):
    ...
    weight[(r >= ri) & (r <= rc)] = 1.0 + 0.5 * (
        1.0 + np.cos(np.pi * ((r[(r >= ri) & (r <= rc)] - ri) / (rc - ri)))
    ) * (g[(r >= ri) & (r <= rc)] - 1.0)
    return weight
...
def norm3d(arr, x, y, z, sig, hx, hy, hz):
    ...
    return (1 / (np.sqrt(2 * np.pi * sig**2) ** 3)) * (
        np.exp(-0.5 * (pbc1d(x - mux, hx) / sig) ** 2)
        * np.exp(-0.5 * (pbc1d(y - muy, hy) / sig) ** 2)
        * np.exp(-0.5 * (pbc1d(z - muz, hz) / sig) ** 2)
    )
...
def convert_density(X, Y, Z, NP, pos, boxl, wdt, hx, hy, hz):
    ...
    for i in range(NP):
        ...
        n_of_r += (1 / (np.sqrt(2 * np.pi * sig**2) ** 3)) * (
            np.exp(-0.5 * (pbcx / sig) ** 2)
            * np.exp(-0.5 * (pbcy / sig) ** 2)
            * np.exp(-0.5 * (pbcz / sig) ** 2)
        )
...
def s_q_from_pos_smear_analytical(q_array, pos, h, rc, rho, nnlist=None):
    ...
    for i in range(q_array.shape[0]):
        ...
        s_array[i] = (
            1.0 + 4.0 * np.pi * rho * int_0_rc + np.sum(sinqr_div_r) / (nparticles)
        )  # np.sum(sinqr_div_r)/(nparticles)
    return s_array.real
            ...
            if ind_need is None:
                if intensity:
                    if q_magnitude is None:
                        raise TypeError(
                            "magnitude of q-space is necessary for computing the intensity"
                        )
...
def s_q_from_pos_smear_array(
    ...
    h = pos[:2, :].T
    boxl = np.mean(h[:, 1] - h[:, 0])
    s_r = s_q_from_pos_smear(
        pos, N=N, wdt=wdt, cs=cs, ms=ms, dump=dump, structure_factor=True
    )
    s_r = correction_grid * s_r
    xxx = np.linspace(N // 2 - ms, N // 2 + ms, 2 * ms + 1, dtype=np.int)
    NN = len(xxx)
...
def s_q_from_pos_smear_par(
    posit, N=200, wdt=500, cs=3, ms=30, ind_need=np.array([0, 2]), dump=False
):
    """Get I(q) from positions on desired indexes of the FFT grid
...
def I_q_from_pos_smear_par(
    ...
    if ISF:
        ...
        p_r = (
            np.exp((2 * np.pi * d_val) ** 2 * (boxl / wdt) ** 2) * p_val
        )  # *form_factor_analytical(2*np.pi*d_val)
    ...
    for i in range(frames):
        for j in range(i, frames):
            if i == j:
                ...
                f = s[j:]
                g = s[i : -(j - i)]
            ts[i, j] = (np.mean(f * g) - np.mean(f) * np.mean(g)) / (
                ((np.mean(f**2) - np.mean(f) ** 2) ** 0.5)
...
def config_repeater(pos_a, h, size=2):
    ...
    for i in range(size):
        for j in range(size):
            for k in range(size):
                new_pos[ct * atoms : (ct + 1) * atoms, :] = pos[
                    :, :
                ] + boxl * np.repeat(np.array([[i, j, k]]), atoms, axis=0)
                ct += 1
...
class test_util:
    def __init__(
        self,
        box_length=17.44024100780622,
        rc=4.0,
        filename="dump.ljtest25k_1000",
        smear=True,
        position="test",
        N_grid=400,
        mini_grid=40,
        ...
        offset_vector=np.array([[0, 0, 1]]),
        default_s_q=0.669314655271,
        q_val=6.28,
        q_tol=0.015,
        total_steps=40,
        timestep=0.005,
    ...
    def generate_configuration(self):
        if self.position == "test":
            ...
            pos = (
                self.offset
                * self.N_grid
                * np.repeat(np.array([[0, 0, 1]]), N_atoms, axis=0)
                + self.box_length / 2
                + np.array(
                    [
                        [1, 0, 0],
            ...
            hin = np.array(
                [[0.0, self.box_length], [0.0, self.box_length], [0.0, self.box_length]]
            )
            ...
            pos, hins, N_atoms = load_dumpfile_atom_data(
                self.filename, 1, 1, verbose=False, h_full=False
            )
            hin = hins[:, :, 0]
            ...
            if self.total_steps < 100:
                pos, hins, N_atoms = load_dumpfile_atom_data(
                    self.filename, self.total_steps, 1, verbose=False, h_full=False
                )
            else:
                pos, hins, N_atoms = load_dumpfile_atom_data_fast(
                    self.filename, self.total_steps, 1, verbose=False, h_full=False
                )
            hin = hins[:, :, 0]
            self.box_length = np.mean(hin[:, 1] - hin[:, 0])
    ...
    def generate_q_grid(self):
        x_grid = np.linspace(
            self.N_grid // 2 - self.mini_grid,
            self.N_grid // 2 + self.mini_grid,
            2 * self.mini_grid + 1,
            dtype=np.int,
        )
        x_grid_global = (
            np.linspace(0, self.N_grid - 1, self.N_grid) * self.box_length / self.N_grid
        )
        ...
        q_grid = (
            ...
            * np.pi
            * np.linspace(
                -(self.N_grid // 2), self.N_grid // 2 - 1, self.N_grid, dtype=np.int
            )
            / self.box_length
        )
        ...
        if self.position == "time":
            point_grid_x = x_grid[self.x0 - smear_width : self.x0 + smear_width + 1] - (
                self.N_grid // 2 - self.mini_grid
            )
            point_grid_y = x_grid[self.y0 - smear_width : self.y0 + smear_width + 1] - (
                self.N_grid // 2 - self.mini_grid
            )
            point_grid_z = x_grid[self.z0 - smear_width : self.z0 + smear_width + 1] - (
                self.N_grid // 2 - self.mini_grid
            )
        ...
        smear_adjust = s_q_from_pos_smear(
            scale_pos,
            self.N_grid,
            self.sigma_grid,
            cs=self.density_cutoff,
            ms=self.mini_grid,
            dump=True,
        ...
        qx = (
            2
            * np.pi
            * (X_grid[self.y0, self.x0, self.z0] - self.N_grid // 2)
            / self.box_length
        )
        qy = (
            2
            * np.pi
            * (Y_grid[self.y0, self.x0, self.z0] - self.N_grid // 2)
            / self.box_length
        )
        qz = (
            2
            * np.pi
            * (Z_grid[self.y0, self.x0, self.z0] - self.N_grid // 2)
            / self.box_length
        )
        qz_array = (
            2
            * np.pi
            * (Z_grid[self.y0, self.x0, :] - self.N_grid // 2)
            / self.box_length
        )
        ...
        q3_array[:, 2] = qz_array
        q_magnitude_array = np.linalg.norm(
            q3_array, axis=1
        ).flatten()  # Scaling q magnitude along the line in q-space
        q_magnitude = np.linalg.norm(
            q3_point, axis=1
        ).flatten()  # Scaling q magnitude at chosen value
        xind = np.linspace(
            self.mini_grid - smear_width,
            self.mini_grid + smear_width,
            2 * smear_width + 1,
            dtype=np.int,
        )
        ...
        [x_weight_grid, y_weight_grid, z_weight_grid] = np.meshgrid(
            x_grid[xind], x_grid[yind], x_grid[zind]
        )
        grid_center = np.array(
            [
                X_grid[self.mini_grid, self.mini_grid, self.mini_grid],
                Y_grid[self.mini_grid, self.mini_grid, self.mini_grid],
                Z_grid[self.mini_grid, self.mini_grid, self.mini_grid],
            ]
        )
        weights = norm3d(
            grid_center / self.box_length,
            x_weight_grid / self.box_length,
            y_weight_grid / self.box_length,
            z_weight_grid / self.box_length,
            sigma_weights,
            self.N_grid // 2,
            self.N_grid // 2,
            self.N_grid // 2,
    ...
    def real_space_s_q_pairwise(self):
        ...
        if self.position != "time":
            ...
            if self.S_Q:
                ...
                self.s_pairwise = (
                    s_pairwise.real
                    * form_factor_analytical(self.q_magnitude_array) ** 2
                )
        else:
            ...
            for i in range(self.total_steps):
                ...
                if i == 0:
                    ...
                    if (
                        np.max(
                            np.linalg.norm(pbc(pos_ext - pos_ref, h_ext, hinv), axis=1)
                        )
                        > self.skin / 2
                    ):
                        nn, index = verletlist(
                            pos_ext, h_ext, self.rv, atoms=self.N_atoms
                        )
                        pos_ref = pos_ext
                r_array = get_r_array(
                    pos_ext, h_ext, self.rc, nnlist=index, atoms_add=self.N_atoms
                ...
                rho = self.N_atoms / np.linalg.det(h)
                # set_num_threads(8)
                s_pairwise[i] = s_q3_from_pos_par(
                    self.q3_point,
                    r_array,
                    self.rc,
                    rho,
            ...
            if self.S_Q:
                ...
                self.s_pairwise = (
                    s_pairwise.real * form_factor_analytical(self.q_magnitude) ** 2
                )
    ...
    def real_space_s_q_position(self):
        ...
        if self.S_Q:
            ...
            self.s_position = (
                s_position.real * form_factor_analytical(self.q_magnitude_array) ** 2
            )
    ...
    def fourier_space(self):
        ...
        grid_s_q = s_q_from_pos_smear(
            ...
            dump=True,
            structure_factor=True,
            correction_grid=self.rescale_factor,
            q_magnitude=self.q_grid_magnitude,
        )
        s_fourier = (
            grid_s_q[
                self.y_global,
        ...
        if self.S_Q:
            ...
            self.s_fourier = (
                s_fourier.real * form_factor_analytical(self.q_magnitude_array) ** 2
            )
        for i in range(self.q_values):
            s_q_integration_grid = np.zeros((2 * self.smear_width + 1) ** 3)
            xind = np.linspace(
                self.x_global - self.smear_width,
                self.x_global + self.smear_width,
                2 * self.smear_width + 1,
                dtype=np.int,
            )
            yind = np.linspace(
                self.y_global - self.smear_width,
                self.y_global + self.smear_width,
                2 * self.smear_width + 1,
                dtype=np.int,
            )
            zind = np.linspace(
                self.N_grid // 2 - self.mini_grid + i - self.smear_width,
                self.N_grid // 2 - self.mini_grid + i + self.smear_width,
                2 * self.smear_width + 1,
                dtype=np.int,
            )
            ...
            integrated_array = (
                np.dot(np.diag(self.weights.flatten()), s_q_integration_grid)
                * self.box_length**-3
            )
        ...
        if self.S_Q:
            ...
            self.s_fourier_smear = (
                s_smear.real * form_factor_analytical(self.q_magnitude_array) ** 2
            )
    ...
    def fourier_space_time(self):
        ...
        if self.smeared_integral:
            ...
            compute_partial = partial(
                ...
                ISF=self.ISF,
                intensity=self.I_Q,
                structure_factor=self.S_Q,
                correction_grid=self.rescale_factor,
                q_magnitude=self.q_grid_magnitude,
                ind_need=self.point_grid,
            )
        s_time = pool.map(
            compute_partial, [self.pos_input[:, :, r] for r in range(self.total_steps)]
        )
        s_time = np.asarray(s_time)
        s_time = s_time.T.real
        if self.smeared_integral:
            integrated_array = (
                (self.I_Q * (form_factor_analytical(self.q_magnitude) ** 2 - 1.0) + 1.0)
                * np.dot(np.diag(self.weights.flatten()), s_time)
                * self.box_length**-3
            )
    ...
    def test_consistency(self, plot_results=False, fig_id=1):
        ...
        print(
            ...
            % (
                self.s_position[self.z0],
                self.s_pairwise[self.z0],
                self.s_fourier[self.z0],
                self.s_fourier_smear[self.z0],
            )
        )
        if self.position == "test":
            error_pos_four = abs(
                (self.default_s_q - self.s_fourier[self.z0]) / self.default_s_q
            )
            error_pair_four = abs(
                (self.default_s_q - self.s_position[self.z0]) / self.default_s_q
            )
        else:
            error_pos_four = abs((self.s_position - self.s_fourier) / self.s_position)
            error_pair_four = abs(
                (self.s_pairwise - self.s_fourier_smear) / self.s_pairwise
            )
        cleared_count = int(error_pos_four.mean() < self.tol) + int(
            error_pair_four.mean() < self.tol
        )
        if cleared_count == 2:
    ...
    def compute_and_save_correlation(self):
        ...
        np.savez(
            "fit_cor_%.2f_%.2f.npz" % (self.q_val, self.atoms_add), b=self.g2_exp_fit
        )
        np.savetxt("ts_cor_%.2f_%.2f.txt" % (self.q_val, self.atoms_add), self.g2_mean)
        np.savetxt(
            "fit_cor_%.2f_%.2f.txt" % (self.q_val, self.atoms_add), self.g2_exp_fit_mean
        )
...
class XPCS_Suite:
    def __init__(
        ...
        if system == "liquid":
            ...
            if self.atom_type is None:
                raise TypeError(
                    "atom_type must be assigned out of ligand or metal: atom_type = 'metal'"
                )
    ...
    def load_and_process_trajectory(self):
        if self.system == "liquid":
            if self.total_steps < 100:
                self.pos, self.hins, self.N_atoms = load_dumpfile_atom_data(
                    self.filename, self.total_steps, 1, verbose=False, h_full=False
                )
            else:
                self.pos, self.hins, self.N_atoms = load_dumpfile_atom_data_fast(
                    self.filename, self.total_steps, 1, verbose=False, h_full=False
                )
            ...
            (
                ...
                self.atoms_a,
                self.atoms_b,
            ) = load_dumpfile_atom_data_binary_fast(
                self.filename,
                self.total_steps,
                1,
                verbose=False,
    ...
    def prepare_intensity_correction(self):
        ...
        if self.intensity:
            self.rescale_factor = s_q_from_pos_smear(
                self.scale_pos,
                N=self.N_grid,
                wdt=self.sigma_grid,
                cs=self.density_cutoff,
        ...
        if self.ISF:
            self.rescale_factor_ISF = s_q_from_pos_smear(
                self.scale_pos,
                N=self.N_grid,
                wdt=self.sigma_grid,
                cs=self.density_cutoff,
    ...
    def prepare_fourier_grid(self):
        ...
        if self.ISF:
            self.correct_grid_ISF = self.rescale_factor_ISF[mini_grid_index].reshape(
                -1, 1
            )
        self.probe_index = np.where(
            abs(self.q_probe.reshape(-1, 1) - self.q_val) < self.q_tol
        )[0]
    ...
    def solve_intensity(self):
        if self.intensity:
            compute_partial = partial(
                ...
                structure_factor=True,
                q_magnitude=self.q_grid_magnitude,
                ind_need=self.probe_index,
            )
            self.s_time = self.pool.map(
                compute_partial,
                [self.pos_input[:, :, r] for r in range(self.total_steps)],
            )
            self.s_time = np.asarray(self.s_time)
            self.s_time = self.s_time.T.real
            self.s_time = np.dot(
                np.diag(self.correct_grid[self.probe_index].flatten()), self.s_time
            )
        if self.ISF:
            compute_partial_ISF = partial(
                s_q_from_pos_smear,
                N=self.N_grid,
                wdt=self.sigma_grid,
                cs=self.density_cutoff,
            ...
            self.s_time_ISF = self.pool.map(
                compute_partial_ISF,
                [self.pos_input[:, :, r] for r in range(self.total_steps)],
            )
            self.s_time_ISF = np.asarray(self.s_time_ISF)
            self.s_time_ISF = self.s_time_ISF.T.real
            self.s_time_ISF = np.dot(
                np.diag(self.correct_grid_ISF[self.probe_index].flatten()),
                self.s_time_ISF,
            )
    ...
    def solve_correlations(self):
        if self.intensity:
            ...
            correl = partial(auto_corr, frames=self.frames)
            self.g2 = self.pool.map(
                correl, [self.s_time[i, :] for i in range(self.items)]
            )
            ...
            self.g2_exp_fit_mean = (
                self.g2_exp_fit_mean - self.g2_exp_fit_mean.min() + 1e-6
            )
            self.g2_mean = np.mean(self.g2, axis=0)
        if self.ISF:
            self.items = self.s_time_ISF.shape[0]
            correl_ISF = partial(ISF_corr, frames=self.frames)
            self.f_qt = self.pool.map(
                correl_ISF, [self.s_time_ISF[i, :] for i in range(self.items)]
            )
    ...
    def save_correlations(self):
        if self.intensity:
            np.savez(
                self.dir_name
                + "ts_cor_%.2f_%d_%s.npz" % (self.q_val, self.atoms_add, self.ext),
                b=self.g2.real,
            )
            np.savez(
                self.dir_name
                + "s_cor_%.2f_%d_%s.npz" % (self.q_val, self.atoms_add, self.ext),
                b=self.s_time.real,
            )
            np.savez(
                self.dir_name
                + "fit_cor_%.2f_%d_%s.npz" % (self.q_val, self.atoms_add, self.ext),
                b=self.g2_exp_fit.real,
            )
            np.savetxt(
                self.dir_name
                + "ts_cor_%.2f_%d_%s.txt" % (self.q_val, self.atoms_add, self.ext),
                self.g2_mean.real,
            )
            np.savetxt(
                self.dir_name
                + "Auto_corr_%.2f_%d_%s.txt" % (self.q_val, self.atoms_add, self.ext),
                self.g2_exp_fit_mean.real,
            )
        if self.ISF:
            np.savetxt(
                self.dir_name
                + "ISF_corr_%.2f_%d_%s.txt" % (self.q_val, self.atoms_add, self.ext),
                self.F_qt,
            )
    ...
    def get_gamma(self):
        ...
        if self.atoms_add < self.N_atoms // 10:
            popt, _ = curve_fit(
                objective_lin, self.times[3:19], np.log(self.g2_exp_fit_mean[3:19])
            )
        else:
            popt, _ = curve_fit(
                objective_lin, self.times[2:6], np.log(self.g2_exp_fit_mean[2:6])
            )
        a, b = popt
...
def end_to_end_pos_to_g2(
    ...
    rescale_factor = s_q_from_pos_smear(
        scale_pos,
        N=N_grid,
        wdt=sigma_grid,
        cs=density_cutoff,
    ...
    rescale_factor_ISF = s_q_from_pos_smear(
        scale_pos,
        N=N_grid,
        wdt=sigma_grid,
        cs=density_cutoff,
    ...
    q_grid = (
        2
        * np.pi
        * np.linspace(-(N_grid // 2), N_grid // 2 - 1, N_grid, dtype=np.int)
        / boxl
    )
    [Qx, Qy, Qz] = np.meshgrid(q_grid, q_grid, q_grid)
    ...
    s_time_ISF = pool.map(
        compute_partial_ISF, [pos_input[:, :, r] for r in range(total_steps)]
    )
    ...
    g2_mean = np.mean(g2, axis=0)
    np.savez(
        "%s_%.2f_%d.npz" % (save_file, q_val, atoms_add),
        g2_m=g2_mean,
        g2_em=g2_exp_fit_mean,
        F=F_qt,
    )
