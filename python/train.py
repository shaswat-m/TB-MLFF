...
class LightDataModule(pl.LightningDataModule):
    def __init__(self, train_params, data_params):
        ...
        self.traj_index_list = range(
            self.traj_index["start"], self.traj_index["end"], self.traj_index["step"]
        )
        self.snap_index_list = range(
            self.snap_index["start"], self.snap_index["end"], self.snap_index["step"]
        )
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        self.train_set, self.test_set = random_split(
            self.dataset, [num_train, num_test], generator
        )

    def train_dataloader(self):
        train_dataloader = GraphDataLoader(
            self.train_set, num_workers=8, batch_size=self.batch_size, shuffle=True
        )
        return train_dataloader

    def val_dataloader(self):
        val_dataloader = GraphDataLoader(
            self.test_set, num_workers=8, batch_size=self.batch_size, shuffle=False
        )
...
class LightModel(pl.LightningModule):
    def __init__(self, train_params, model_params):
        ...
        self.avg_force = nn.Parameter(torch.tensor([0.0]), requires_grad=False)
        self.std_force = nn.Parameter(torch.tensor([1.0]), requires_grad=False)
    ...
    def training_step(self, batch, batch_idx):
        ...
        # log
        self.log(
            "train mae", loss, on_step=True, on_epoch=True, prog_bar=True, logger=True
        )
        self.log(
            "avg_force",
            self.avg_force[0],
    ...
    def validation_step(self, batch, batch_idx):
        ...
        with torch.no_grad():
            ...
            # log
            self.log(
                "val mae", mae, on_step=True, on_epoch=True, prog_bar=True, logger=True
            )
            self.log(
                "val mse", mse, on_step=True, on_epoch=True, prog_bar=True, logger=True
            )
...
def main(inputs):
    ...
    # define data module
    dm = LightDataModule(train_params=train_params, data_params=data_params)
    dm.setup()
    ...
    # prepare logger
    wandb_logger = pl.loggers.WandbLogger(project="mdnet", log_model=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input-file", help="input file", type=str, default="train_info.json"
    )
